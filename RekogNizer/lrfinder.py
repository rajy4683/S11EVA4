...
class LRFinder(object):
    ...
    def range_test(
        ...
        # Reset test results
        self.history = {"lr": [], "loss": [], "acc": [], "val_acc":[]}
        self.best_loss = None
        self.iter_grp = 100
        ...
        if step_mode.lower() == "exp":
            lr_schedule = ExponentialLR(self.optimizer, end_lr, num_iter//self.iter_grp)
        elif step_mode.lower() == "linear":
            lr_schedule = LinearLR(self.optimizer, end_lr, num_iter//self.iter_grp)
        else:
        ...
        total_iter = num_iter * len(train_loader)
        for iteration in tqdm(range(num_iter)):
            # Train on batch and retrieve loss
            val_acc=0.0
            loss,acc = self._train_batch_acc(iter_wrapper, accumulation_steps)
            #loss = self._train_batch(iter_wrapper, accumulation_steps)
            # Update the learning rate
            if (iteration != 0 and iteration % self.iter_grp == 0):
                curr_lr = lr_schedule.get_lr()[0]
                self.history["lr"].append(curr_lr)
                if val_loader:
                    loss,val_acc = self._validate(val_loader)
                    print("Epoch:{} val_acc:{} loss:{} lr:{} ".format(iteration/self.iter_grp, val_acc,loss, curr_lr))
                    self.history["loss"].append(loss)
                    #self.history["acc"].append(acc)
                    self.history["val_acc"].append(val_acc)
                    
                    #self.history["val_acc"].append([val_acc]*self.iter_grp)
                lr_schedule.step()
                #curr_lr = lr_schedule.get_lr()[0]
    ...
    def _train_batch_acc(self, iter_wrapper, accumulation_steps):
        self.model.train()
        total_loss = None  # for late initialization
        ...
        for i in range(accumulation_steps):
            ...
            #print(train_accuracy, len(inputs))
            # Loss should be averaged in each step
            loss /= accumulation_steps
            train_accuracy /= accumulation_steps
            ...
            train_accuracy = 100.0*(train_accuracy/len(inputs))
        ...
        return total_loss.item(), total_train_accuracy
    ...
    def plot_acc(self, skip_start=0, skip_end=0, log_lr=True, show_lr=None):
        """Plots the learning rate range test.
